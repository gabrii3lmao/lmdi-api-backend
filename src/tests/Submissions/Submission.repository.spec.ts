...
describe("SubmissionRepository", () => {
  ...
  it("deve atualizar status e nota com returnDocument: 'after'", async () => {
    ...
    vi.mocked(Submission.findByIdAndUpdate).mockResolvedValue(updateData as any);
  ...
  it("deve buscar submissões por classId", async () => {
    vi.mocked(Submission.find).mockResolvedValue([{ _id: "1" }] as any);
  ...
  it("deve deletar múltiplas submissões por examId", async () => {
    vi.mocked(Submission.deleteMany).mockResolvedValue({ deletedCount: 3 } as any);
